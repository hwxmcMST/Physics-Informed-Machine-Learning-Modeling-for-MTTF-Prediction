...
K=NE2K_3dTruss(Node, Element, e, A); % assemble the global stiffness matrix
...
% nS = size(P,1); % the load can have many random samples
f=zeros(63,1);% load vector
P=-P;% all the load follows -Z direction , so minus the magnitude.
f(3*(1:13))=P;
u=K\f;
g=u(3);% the z displacement of Node 1 
